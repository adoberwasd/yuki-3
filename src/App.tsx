...
const ADMIN_USERNAME = "samarskiyyyy";
...
export default function App() {
  ...
  const [orders, setOrders] = useState<OrderItem[]>(() => {
    if (typeof window === "undefined") return [];
    try {
      const s = window.localStorage.getItem("yuki_orders");
      return s ? (JSON.parse(s) as OrderItem[]) : [];
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    window.localStorage.setItem("yuki_orders", JSON.stringify(orders));
  }, [orders]);
  ...
  useEffect(() => {
    ...
    if (u) {
      ...
      setUser(next);
      if (
        next.username?.toLowerCase() === DEBUG_USERNAME &&
  ...
  // Nav items: добавляем админку только для админа
  ...
  const handleIPaid = () => {
    ...
    setCart([]);
    setPaymentFlow({ ...paymentFlow, stage: "waiting", orderId: newOrder.id });
  };

  // ─── Админка: принять/отклонить заказ ───
  const adminUpdateOrder = (id: number, status: OrderStatus) => {
    vibrate("medium");
    ...
    <div className="space-y-5">
      <SectionCard className="bg-[linear-gradient(135deg,rgba(232,121,249,0.18),rgba(99,102,241,0.12))]">
        ...
        <p className="mt-2 text-sm leading-6 text-violet-100/70">
          Здесь только заказы со статусом «в обработке». Принимай или отклоняй после проверки
          реквизитов.
        </p>
        <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-violet-100/65">
